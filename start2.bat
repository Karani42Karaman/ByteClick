...
:: CMD ekran�n� UTF-8 moduna al�yoruz
chcp 65001 >nul
...
echo Links will be emailed automatically once generated.
echo ---------------------------------------------------
echo.

:: Eski ge�ici log dosyalar�n� temizle
...
:: 30 saniyelik saya� ve link kontrol d�ng�s� (Tamamen Batch tabanl�)
set /a counter=0
set "LINK1="
set "LINK2="
...
:send_mail
echo.
echo  Found Link 1: %LINK1%
echo  Found Link 2: %LINK2%
echo.
echo Sending email via SMTP...

:: PowerShell sadece d�z bir mail g�nderme arac� olarak �a�r�l�r (Hi�bir d�ng� veya operat�r i�ermez)
powershell -NoProfile -ExecutionPolicy Bypass -Command "$smtp = New-Object Net.Mail.SmtpClient('%SMTP_SUNUCU%', %SMTP_PORT%); $smtp.EnableSsl = $true; $smtp.Credentials = New-Object Net.NetworkCredential('%GONDERICI_MAIL%', '%MAIL_SIFRESI%'); $msg = New-Object Net.Mail.MailMessage; $msg.From = '%GONDERICI_MAIL%'; $msg.To.Add('%ALICI_MAIL%'); $msg.Subject = 'ByteClick Live Tunnel Links'; $msg.Body = '<h3>ByteClick Tunnels Successfully Opened!</h3><p>Active TradingView Webhook endpoints:</p><ul><li><a href=\"%LINK1%\">%LINK1%</a></li><li><a href=\"%LINK2%\">%LINK2%</a></li></ul>'; $msg.IsBodyHtml = $true; try { $smtp.Send($msg); Write-Host 'Email sent successfully!' -ForegroundColor Green; } catch { Write-Host 'Email Error' -ForegroundColor Red; }"
goto stream_logs
